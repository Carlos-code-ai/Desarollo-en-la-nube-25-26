...
const PriceRangeSlider = ({ max, value, onChange }) => {
    ...
    // Estilos en línea para ocultar el thumb del input range
...
const FilterPanel = ({ isOpen, onClose, availableFilters, activeFilters, onFilterChange, onResetFilters, priceFilter, onPriceChange, maxPrice }) => {
    ...
    return (
        <div className={`fixed inset-0 z-50 ${isOpen ? 'pointer-events-auto' : 'pointer-events-none'}`}>
            <div className="filter-backdrop fixed inset-0 bg-black/30 opacity-0 backdrop-blur-sm" onClick={onClose} />
            <div ref={panelRef} className="fixed top-0 right-0 h-full w-full max-w-md bg-surface-container shadow-2xl flex flex-col transform translate-x-full rounded-l-2xl overflow-hidden">
                ...
                <div className="p-4 border-t border-outline flex items-center gap-4 bg-surface-container">
                    <button onClick={onResetFilters} className="w-full h-12 rounded-full border border-outline text-on-surface-variant font-bold hover:bg-surface-container-high transition-colors text-sm">Limpiar Filtros</button>
                    <button onClick={onClose} className="w-full h-12 rounded-full bg-primary text-on-primary font-bold hover:bg-primary/90 transition-colors text-sm">Ver Resultados</button>
                </div>
            </div>
        </div>
    );
