...
const RentalCard = ({ rental }) => {
  ...
  return (
    <div onClick={chatId ? handleGoToChat : undefined} className="flex items-start sm:items-center gap-4 bg-surface p-4 rounded-2xl shadow-sm hover:bg-surface-container transition-colors duration-200 cursor-pointer">
      ...
      <div className="flex-grow">
        ...
        {isOwnerView && (
          <div className="flex items-center gap-2 mt-2">
            <img src={renterPhotoURL || '/avatar-placeholder.png'} alt={renterName || 'Usuario'} className="w-6 h-6 rounded-full object-cover" />
            <p className="text-xs text-on-surface-variant">Solicitado por {renterName}</p>
          </div>
        )}
      </div>
      <div className="flex flex-col items-end justify-between self-stretch py-1">
        <span className={`px-3 py-1 text-xs font-bold rounded-full ${getStatusChipStyle(status || 'default')}`}>
